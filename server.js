...
import serviceAccountkey from "./react-js-blog-a2230-firebase-adminsdk-fbsvc-b656265ce8.json" with { type:"json" }
...
server.use(cors())
server.get("/api/imagekit/signature", (req, res) => {
...
/*const generateUploadUrl =async () =>
{
    ...
    return await imagekit.getSignedUrlPromise('putObject',{
...
// validating data from frontend
   ...
   if(!passwordRegex.test(password)){
    return res.status(403).json({"error":"password should be 6 to 12 characters with numeric with one lowercase and one uppercase letter"})
   }
   ...
   bcrypt.hash(password, 10, async (err, hashed_password) => {
    ...
    .catch(err => {
        ...
        if(err.code == 11000) {
            ...
            return res.status(500).json({"status":"Email already Exists"})
...
server.post("/change-password", verifyJWT, (req,res) => {
    ...
    let { currentPassword, newPassword } = req.body;
      ...
      if(!passwordRegex.test(currentPassword) || !passwordRegex.test(newPassword)){
          return res.status(403).json({error: "Password must be 6 to 20 characters long with a numeric, 1 lowercase and 1 uppercase letter"})  
        }
        ...
        .then((user) => {
            ...
            bcrypt.compare(currentPassword, user.personal_info.password, (err, result) => {
                if(err) {
                    return res.status(200).json({ error: "Some error occured while changing the password, please try again later"})
                }
                if(!result){
...
server.post("/update-profile",verifyJWT,(req,res) => {
    ...
    let bioLimit = 150;
      ...
      if(username.length < 3){
                return res.status(403).json( { error: "Username must atleast 3 characters long"})
            }
      ...
      if(bio.length > bioLimit) {
        ...
        try{
            ...
            return res.status(500).json({error: "You must provide full links including http(s)"})
        }  
        let UpdateObj = {
...
server.post('/create-blog', verifyJWT, (req,res) => {
   ...
   //if(isAdmin){
     ...
     if(!title.length){
        return res.status(403).json({error:"You must provider a title to publish the blog"});
    }
    ...
    if(!content.blocks.length){
        return res.status(403).json({error:"There must be content to piblish it"});

    }
    if(!tags.length || tags.length > 10){
        return res.status(403).json({error:"Please provide tags "})
    ...
    .catch(err => {
        return res.status(403).json({error:"Failed to update total post"})
    })
...
//return res.status(500).json({ error: "you dont have permission to create any blog"})
  // }
...
server.post("/add-comment", verifyJWT, (req,res) => {
    ...
    console.log("req.body at add-comment:", req.body);

     if(!comment.length){
        return res.status(403).json({ error: "Need to Write Something!!"});
    }
...
const deleteComments = (_id) => {
    ...
    .then(comment => {
        ...
        Notification.findOneAndDelete({ comment: _id}).then(notification => console.log('comment notification delted'))
...
server.post("/delete-blog", verifyJWT, (req,res) => {
    ...
    //else {
    //return res.status(500).json({ error: "you dont have permission to delete any blog"})
  // }
...
//4EaeNlFMjb1zFlv8
//mongodb+srv://muskan27018:<db_password>@cluster0.e4fjmqn.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0