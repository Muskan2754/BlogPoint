...
export const fetchComments = async ({ skip = 0 , blog_id, setParentCommentCountFun, comment_arr = null }) => {
    let res;
...
const CommentsContainer = () => {
    ...
    const loadMoreComments = async () => {
        ...
        let newCommentsArr = await fetchComments({ skip: totalParentCommentsLoaded, blog_id: _id, setParentCommentCountFun: setTotalParentCommentsLoaded, comment_arr: commentsArr})
    ...
    return (
        <div className={"max-sm:w-full fixed " + ( commentsWrapper ?   " top-0 sm:right-0 " : " top-[100%] sm:right-[-100%] " ) + " duration-700 max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden"}>
            <div className="relative">
                ...
                <p className="text-m mt-2 w-[70%] text-dark-grey line-clamp-1">{title}</p>
            ...
            <CommentField action="comment"/>
            {
                ...
                }) : <NoDataMessage message="no commment" />
            }
